...
{
    // Метод расширения для PlayerInventory
    public static class PlayerInventoryExtensions
    {
        public static IEnumerable<Item> AllItems(this PlayerInventory inventory)
        {
            if (inventory == null) return Enumerable.Empty<Item>();
            ...
            if (inventory.containerBelt != null)
                items.AddRange(inventory.containerBelt.itemList);
                
            if (inventory.containerWear != null)
                items.AddRange(inventory.containerWear.itemList);

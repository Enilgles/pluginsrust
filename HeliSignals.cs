...
namespace Oxide.Plugins
{
    ...
    public class HeliSignals: RustPlugin
    {
        ...
        [PluginReference] Plugin Friends, Clans, FancyDrop, RewardPlugin, ServerRewards, Economics, IQEconomic, Vanish, NoEscape, BotReSpawn, BetterNPC, DynamicPVP, XPerience, SkillTree, NpcSpawn;
        ...
        {
            if (config?.rewards == null || config?.purchasing == null)
            {
                PrintError("Configuration is null! Please check your config file.");
                return;
            }

            // Проверяем и устанавливаем плагин наград в зависимости от конфигурации
            if (config.rewards.enableRewards && !string.IsNullOrEmpty(config.rewards.rewardPlugin))
            {
                switch (config.rewards.rewardPlugin.ToLower())
                {
                    case "serverrewards":
                        RewardPlugin = plugins.Find("ServerRewards");
                        if (RewardPlugin == null)
                            PrintError("ServerRewards plugin not found - rewards will not work!");
                        break;
                        
                    case "economics":
                        RewardPlugin = plugins.Find("Economics");
                        if (RewardPlugin == null)
                            PrintError("Economics plugin not found - rewards will not work!");
                        break;
                        
                    case "iqeconomic":
                        RewardPlugin = plugins.Find("IQEconomic");
                        if (RewardPlugin == null)
                            PrintError("IQEconomic plugin not found - rewards will not work!");
                        break;
                }
            }

            // Проверяем плагин для покупок
            if (!string.IsNullOrEmpty(config.purchasing.defaultCurrency))
            {
                switch (config.purchasing.defaultCurrency.ToLower())
                {
                    case "serverrewards":
                        if (plugins.Find("ServerRewards") == null)
                            PrintError("ServerRewards plugin not found - buying will not work!");
                        break;
                        
                    case "economics":
                        if (plugins.Find("Economics") == null)
                            PrintError("Economics plugin not found - buying will not work!");
                        break;
                        
                    case "iqeconomic":
                        if (plugins.Find("IQEconomic") == null)
                            PrintError("IQEconomic plugin not found - buying will not work!");
                        break;
                }
            }

            // Проверяем плагин XP если включено
            if (config.rewards.enableXP && !string.IsNullOrEmpty(config.rewards.pluginXP))
            {
                switch (config.rewards.pluginXP.ToLower())
                {
                    case "skilltree":
                        if (plugins.Find("SkillTree") == null)
                            PrintError("SkillTree plugin not found - XP rewards will not work!");
                        break;
                        
                    case "xperience":
                        if (plugins.Find("XPerience") == null)
                            PrintError("XPerience plugin not found - XP rewards will not work!");
                        break;
                }
            }
        }
        ...
        private void GiveReward(ulong playerId, double amount, string heliProfile)
        {
            ...
            switch (config.rewards.rewardPlugin.ToLower())
            {
                ...
                case "economics":
                    ...
                    break;
                case "iqeconomic":
                    RewardPlugin?.Call("API_SET_BALANCE", playerId, amount, 0);
                    Message(player, "PointsGiven", config.rewards.rewardUnit, (int)amount, heliProfile);
                    break;
                default:
                    break;
                ...
                case "iqeconomic":
                    {
                        ...
                        // Получаем баланс через IQEconomic плагин напрямую
                        var iqEconomic = plugins.Find("IQEconomic");
                        if (iqEconomic == null)
                        {
                            PrintError("IQEconomic plugin not found!");
                            return;
                        }

                        var balanceObj = iqEconomic.Call("API_GET_BALANCE", basePlayer.userID.ToString());
                        if (balanceObj == null)
                        {
                            PrintError($"Failed to get balance for player {basePlayer.displayName} - balance is null");
                            return;
                        }
                        ...
                        try 
                        {
                            balance = Convert.ToDouble(balanceObj);
                        }
                        catch (Exception ex)
                        {
                            PrintError($"Failed to convert balance for player {basePlayer.displayName}: {ex.Message}");
                            return;
                        }
                        ...
                        if (balance >= cost)
                        {
                            if (GiveHeliSignal(basePlayer, skinId, heliProfile, 1, "purchase"))
                            {
                                // Используем правильный формат вызова API для снятия денег
                                iqEconomic.Call("API_REMOVE_BALANCE", basePlayer.userID.ToString(), cost);
                                Message(player, "Receive", 1, heliProfile);
                                return;
                            }
                            else
                        ...
                        {
                            Message(player, "CantAfford", $"{cost} Scrap", $"{cost - balance} Scrap");
                            return;
                        }
                    }
                    break;
        ...
        private class ConfigData
        {
            ...
            {
                [JsonProperty(PropertyName = "Rewards Plugin (ServerRewards | Economics | IQEconomic)")]
            ...
            public class Purchasing
            {
                ...
                [JsonProperty(PropertyName = "Purchasing Currency (ServerRewards|Economics|IQEconomic|Custom)")]
